...
RCT_EXPORT_SYNCHRONOUS_TYPED_METHOD(RCTArrayBuffer *, testMethodWhichTransformsArrayBuffer : (RCTArrayBuffer *)buffer)
{
...
RCT_EXPORT_SYNCHRONOUS_TYPED_METHOD(RCTArrayBuffer *, testMethodWhichReturnsItsArgument : (RCTArrayBuffer *)buffer)
{
  auto *bytes = static_cast<uint8_t *>(buffer.mutableBytes);
...
RCT_EXPORT_SYNCHRONOUS_TYPED_METHOD(NSNumber *, testMethodWhichChecksArrayBufferAliasing : (RCTArrayBuffer *)buffer)
{
...
RCT_EXPORT_METHOD(testMethodWhichStoresArrayBuffer : (RCTArrayBuffer *)payload)
{
  self.lastReceivedPayload = [NSData dataWithBytes:payload.mutableBytes length:payload.length];
}

RCT_EXPORT_METHOD(testMethodWhichCallsBackWithArrayBuffer : (double)size callback : (RCTResponseSenderBlock)callback)
{
