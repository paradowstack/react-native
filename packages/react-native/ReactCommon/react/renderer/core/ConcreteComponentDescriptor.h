...
class ConcreteComponentDescriptor : public ComponentDescriptor {
 ...
 public:
...
#ifdef RN_SERIALIZABLE_STATE
      const auto& dynamic = fallbackToDynamicRawPropsAccumulation
          ? shadowNodeProps->rawProps
          : static_cast<folly::dynamic>(rawProps);
#else
  ...
  Props::Shared cloneResolvedProps(
    ...
    shadowNodeProps->sweepCalcExpressions();
    auto resolver =
        DynamicResolver(shadowNodeProps->calcExpressions, resolveContext);
    shadowNodeProps->resolveProperties(resolver);
    return shadowNodeProps;
  }
